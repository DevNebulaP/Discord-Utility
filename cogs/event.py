                ...
                for i in data:
                    if i['message_id'] == payload.message_id:
                        ...
                        if payload.emoji.name == "✅":
                            ...
                            if field == "None": field = ""
                            new_embed.set_field_at(index=0, name="Accepted", value=field+"\n"+payload.member.name)             
                        elif payload.emoji.name == "❌":
                            field = new_embed.fields[1].value
                            if field == "None": field = ""
                            new_embed.set_field_at(index=1, name="Declined", value=field+"\n"+payload.member.name)
                        elif payload.emoji.name == "❔":
                            field = new_embed.fields[2].value
                            if field == "None": field = ""
                            new_embed.set_field_at(index=2, name="Maybe", value=field+"\n"+payload.member.name)
            ...
            for i in data:
                if i['message_id'] == payload.message_id:
                    ...
                    if payload.emoji.name == "✅":
                        ...
                        if not len(field): field = ["None"]
                        new_embed.set_field_at(index=0, name="Accepted", value="\n".join(field))             
                    elif payload.emoji.name == "❌":
                        field = new_embed.fields[1].value
                        field = field.split("\n")
                        field.remove(user.name)
                        if not len(field): field = ["None"]
                        new_embed.set_field_at(index=1, name="Declined", value="\n".join(field))
                    elif payload.emoji.name == "❔":
                        field = new_embed.fields[2].value
                        field = field.split("\n")
                        field.remove(user.name)
                        if not len(field): field = ["None"]
                        new_embed.set_field_at(index=2, name="Maybe", value="\n".join(field))
                    await msg.edit(embed=new_embed)
                    return
